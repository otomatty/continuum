...
use crate::concepts::user::{initialize_mock_users, get_user_by_username};
use crate::concepts::contribution::{initialize_mock_contribution_graph, initialize_mock_repository_contributions};
...
pub fn PortfolioPage() -> impl IntoView {
    ...
    let contribution_graph_weekly = move || {
        initialize_mock_contribution_graph(&username(), Period::Weekly)
    };
    
    let contribution_graph_monthly = move || {
        initialize_mock_contribution_graph(&username(), Period::Monthly)
    };
